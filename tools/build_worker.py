...
async function captureRequests(request, env, url) {
  ...
  if (status === "pending") {
    const result = await env.DB.prepare("SELECT * FROM capture_requests WHERE status = 'pending' ORDER BY requested_at LIMIT 10").all();
...
async function captureRequestById(request, env, url, id) {
  if (!env.DB) return reply({error: "抓取服务暂时不可用"}, 503);
  const selectSql = "SELECT * FROM capture_requests WHERE id = ? LIMIT 1";
