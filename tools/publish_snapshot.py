...
CATEGORY = {
    ...
    "url": "https://www.amazon.com/gp/new-releases/fashion/2368365011",
}
...
def archive_path(public_root: Path, date: str) -> Path:
    year, month, _ = date.split("-")
    return public_root / "data" / "daily" / year / month / f"{date}.json"


def read_existing_snapshots(public_root: Path, before_date: str) -> list[dict]:
    daily_root = public_root / "data" / "daily"
    snapshots = []
...
def rebuild_manifest(public_root: Path) -> dict:
    entries = []
    for path in (public_root / "data" / "daily").glob("*/*/*.json"):
        payload = json.loads(path.read_text(encoding="utf-8"))
        entries.append({
    ...
    atomic_json(public_root / "data" / "manifest.json", manifest)
    return manifest


def publish(input_path: Path, snapshot_date: str, captured_at: str, detail_source: str) -> dict:
    ...
    earlier = read_existing_snapshots(PUBLIC_ROOTS[0], snapshot_date)
    ...
    for public_root in PUBLIC_ROOTS:
        destination = archive_path(public_root, snapshot_date)
        atomic_json(destination, snapshot)
        atomic_json(public_root / "data" / "latest.json", snapshot)
        manifest = rebuild_manifest(public_root)
        atomic_json(public_root / "data" / "status.json", {
            "status": "ok",
    ...
    return {"date": snapshot_date, "items": len(items), "quality": quality}
...
def main() -> None:
    ...
    result = publish(args.input.resolve(), args.date, captured_at, args.detail_source)
