...
3. 对每个 ASIN 至少执行一次 Ecomtool MCP 详情采集，逐项补齐产品图、品牌、标题、价格、促销、上架时间、BSR 大类与全部可见小类排名。上传来源可将 100 个 ASIN 一次传给 `amazon_get_product_info` 和 `amazon_get_market_analysis_data`，等待两个结果网址真正各返回 100 个匹配 ASIN 后，运行 `python published-dashboard/tools/merge_uploaded_ecomtool.py --ranking <下载JSON> --product-info <商品详情结果网址> --market-analysis <市场调研结果网址> --output published-dashboard/work/merged-<日期>.json --node <节点>`。缺失信息必须如实标记，并保留 `detailStatus` 与 `detailAttempts`。市场调研的“上架日期”不能冒充 Amazon 详情页的 Date First Available。
4. 运行现有校验，并用 `python published-dashboard/tools/check_snapshot_images.py <合并JSON>` 检查全部 100 张图片。只有名次 1–100 完整、ASIN 唯一、标题和图片有效、100 个商品都完成详情采集，且详情来源为 Ecomtool MCP 时才发布。图片与导出 ID 不同时保留差异标记，以当前 Ecomtool 商品图为准。
